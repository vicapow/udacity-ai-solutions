...
def run(grid, goal, spath, params, printflag = False, speed = 0.1, timeout = 1000):
    ...
    while not myrobot.check_goal(goal) and N < timeout:
        ...
        def vec_minus(v1, v2): return [v1[0] - v2[0], v1[1] - v2[1]]
        def vec_len(v1): return sqrt(v1[0]**2 + v1[1]**2)
        def vec_scale(v, s): return [v[0] * s, v[1] * s]
        def vec_unit(v): return vec_scale(v, 1. / vec_len(v))

        p1 = spath[index]
        if index + 1 < len(spath): p2 = spath[index + 1]
        else: p2 = vec_add(p1, vec_unit(vec_minus(spath[index], spath[index - 1])))

        delta_x = vec_minus(p2, p1)
        r = vec_minus(estimate, p1)
        cte = ( r[1] * delta_x[0] - r[0] * delta_x[1] ) / ( delta_x[0] ** 2 + delta_x[1] ** 2)

        if vec_dot(r, delta_x) / vec_dot(delta_x, delta_x) > 1: index += 1
