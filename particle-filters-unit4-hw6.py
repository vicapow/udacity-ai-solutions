...
class robot:
    ...
    def move(self, motion): # Do not change the name of this function
        ...
        # apply noise to motion distance and angel
        a = random.gauss(motion[0], steering_noise) % (2. * pi)
        d = random.gauss(motion[1], distance_noise)
        b = (d / self.length) * tan(a)
        theta = self.orientation
        if(b > 0.0001):
            R = d / b
            x = self.x - sin(theta) * R + sin(theta + b) * R
            y = self.y + cos(theta) * R - cos(theta + b) * R
            theta = (theta + b ) % (2 * pi)
        else:
            x = self.x + cos(theta) * d
            y = self.y + sin(theta) * d
    ...
    def sense(self, add_noise = 1): #do not change the name of this function
        ...
        for landmark in landmarks:
            x = landmark[1]
            y = landmark[0]
            bearing = atan2(y - self.y, x - self.x) - self.orientation
            if add_noise:
